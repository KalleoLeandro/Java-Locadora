...
/**
 * Classe responsável por armazenar as telas e componentes do processo de consulta de filmes
...
{//inicio da classe		
	...
	private String colunastabela [] = {"Código", "Nome", "Valor", "Disponível", "Promoção", "Valor Promocional"};
	...
	{//inicio do método
		...
		for(Filme filme : new FilmeController().getFilmes())
		{
			...
			modelo.addRow(new String[] {filme.getCodigo()+"",filme.getNome(), filme.getValor()+"", filme.isDisponivel() ? "SIM" : "NÃO", filme.isPromocao() ? "SIM" : "NÃO",filme.getValorPromocao()+ ""});				
